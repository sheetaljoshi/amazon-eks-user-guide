...
+
You can also replace [.replaceable]`amazon-linux-2023` with a different AMI type. For more information, see <<retrieve-ami-id>>.
+
...
. When your stack has finished creating, select it in the console and choose *Outputs*. If you are using the `EKS API` or `EKS API and ConfigMap` Authentication Modes, this is the last step.
. If you are using the `ConfigMap` Authentication Mode, record the *NodeInstanceRole* for the node group that was created.
