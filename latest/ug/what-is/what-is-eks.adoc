...
<<capabilities,Amazon EKS Capabilities>> are fully managed cluster services that extend your cluster's functionality with hands-free Kubernetes-native tools, including:
...
To learn more, see <<capabilities>>.
...
* For Amazon EKS cluster, Amazon EKS Auto Mode, Amazon EKS Capabilities, and Amazon EKS Hybrid Nodes pricing, see link:eks/pricing/[Amazon EKS Pricing,type="marketing"].
